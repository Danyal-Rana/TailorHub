...
import { LayoutDashboard, ShoppingBag, Users, LogOut, Scissors, Package, Calendar, Settings as SettingsIcon, X } from 'lucide-react';
            ...
            `}>
                ...
                <div className="p-8 flex items-center justify-between border-b border-white/5">
                    <div className="flex items-center space-x-4">
                        ...
                        <span className="text-2xl font-black tracking-tight bg-gradient-to-br from-white to-indigo-300 bg-clip-text text-transparent">Tailors Hub</span>
                    </div>
                    {/* Close button for mobile */}
                    <button onClick={onClose} className="md:hidden text-indigo-200 hover:text-white p-2 rounded-xl hover:bg-white/10 transition-colors">
                        <X className="h-6 w-6" />
                    </button>
                </div>
                ...
                <nav className="flex-1 px-4 py-8 space-y-3 overflow-y-auto">
                    {navItems.map((item) => (
                        <NavLink
                            ...
                            className={({ isActive }) =>
                                `flex items-center space-x-4 px-5 py-4 rounded-2xl transition-all duration-300 group ${isActive
                                    ? 'bg-white/10 text-white shadow-lg border border-white/10'
                                    : 'text-indigo-200/70 hover:bg-white/5 hover:text-white'
                                }`
                            }
                        >
                            <item.icon className={`h-5 w-5 transition-transform duration-300 group-hover:scale-110 ${window.location.pathname === item.path ? 'text-white' : 'text-indigo-400 group-hover:text-white'}`} />
